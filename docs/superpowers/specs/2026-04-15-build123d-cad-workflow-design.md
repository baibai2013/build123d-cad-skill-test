...
**几何对齐（Step 1.5）— 5种方案，均由用户主动触发**：

| 方案 | 触发词 | 生成内容 | 最适合 |
|------|--------|---------|-------|
| A 3视图草图 | 「画草图」「三视图」「sketch」 | Matplotlib PNG（正/侧/俯） | 复杂单体，无参考图 |
| B OCP快速原型 | 「先看比例」「占位块」「proxy」 | build123d Box/Cylinder 占位 | 多部件装配 |
| C 关键截面草图 | 「画截面」「profile」「旋转轮廓」 | Matplotlib 截面轮廓 PNG | Revolve/Sweep 件 |
| D 参考图标注 | 「解读图」「标注图」「有参考图」 | 纯文字标注+尺寸推断 | 用户有参考图时 |
| E 参数约束表 | 「先确认参数」「列参数」「参数合同」 | Markdown 表格+确认门 | 精度配合件 |

建模前必须询问用户是否有参考图（有则走方案 D）。可组合：多部件推荐 B+D+E 三连。
